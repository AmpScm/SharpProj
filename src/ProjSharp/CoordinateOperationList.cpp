...
int CoordinateOperationList::SuggestedOperation(...array<double>^ coordinate)
{
	PJ_COORD coord;
...
array<double>^ CoordinateOperationList::DoTransform(bool forward, array<double>^ coordinate)
{
	if (!coordinate)
	...
	for (int iRetry = 0; iRetry <= N_MAX_RETRY; iRetry++)
	{
		...
		if (proj_errno(c) == -62 /*PJD_ERR_NETWORK_ERROR*/) {
			return FromCoordinate(res, coordinate->Length);
		}
